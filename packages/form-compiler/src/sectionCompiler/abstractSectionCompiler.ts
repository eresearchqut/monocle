...
export abstract class AbstractSectionCompiler {
    schemaProperties = (form: Form, section: Section): { [property: string]: JsonSchema | undefined } | undefined =>
        section.inputs
    ...
    requiredProperties = (form: Form, section: Section): string[] =>
        section.inputs
            ? section.inputs
                  .map((input: Input) => {
                      const propertyName = generatePathFromName(input.name);
    ...
    uiElements = (form: Form, section: Section): UISchemaElement[] | undefined =>
        section.inputs
            ? section.inputs
