...
export const InputCalendarCell = (props: CellProps) => {
    ...
    const {locale, hideCalendarIcon, hourFormat, includeSeconds, includeMilliseconds, stepHours, stepMinutes, stepSeconds, stepMilliseconds} = merge({}, config, uischema.options) as InputDateCellOptions;
    ...
    const dateFormat = new Intl.DateTimeFormat(locale).formatToParts(new Date())
            .map(obj => {
                switch (obj.type) {
                    case "day":
                        return "dd";
