...
export const SampleContainer: FunctionComponent<SampleContainerProps> = (props) => {
    ...
    return (
        <div className="biobank-sample-container-grid p-d-flex p-jc-center">
            <div className="biobank-sample-container-grid-inside" style={style}>
                <Tooltip target=".biobank-sample-container-grid-cell.full" mouseTrack={true} position="top" />
                {rows.map((rowIdx) => {
                    const highlightRow = highlightedSample && highlightedSample.row === rowIdx;
                    const extraRowClasses = highlightRow ? 'highlight' : '';
                    return (
                        <div key={rowIdx} className={`biobank-sample-container-grid-row ${extraRowClasses}`}>
                            {cols.map((colIdx) => {
...
const Cell: FunctionComponent<CellProps> = (props) => {
    ...
    const RowLabel: FunctionComponent = () => (
        <span className="biobank-sample-cell-coord biobank-sample-cell-coord-y p-text-secondary">
            {colLabelNames(props.rowIdx)}
        </span>
    );
    const ColumnLabel: FunctionComponent = () => (
        <span className="biobank-sample-cell-coord biobank-sample-cell-coord-x p-text-secondary">
            {rowLabelNames(props.colIdx)}
        </span>
    );
    ...
    return (
        <div
            className={`biobank-sample-container-grid-cell ${extraCellClasses}`}
