...
export const InputControl = (props: ControlProps) => {
  const { id, schema, label, uischema, visible, path, required, cells, errors } = props;
  ...
  const dispatchCell = <DispatchCell uischema={uischema} schema={schema} path={path} cells={cells} id={id} />;
  ...
  return (
    <div className="p-field p-fluid">
      <label htmlFor={id} aria-required={required}>
        ...
        {required && (
          <i
            className="tooltip pi pi-exclamation-circle p-text-secondary p-overlay-badge p-ml-1"
      ...
      {description && <div className="p-text-light p-mb-2">{description}</div>}
      {dispatchCell}
      {errors && (
        <small id={`${id}-error`} className="p-text-light p-error p-mt-2">
          {errors}
        </small>
      )}
    </div>
