...
const PanelRenderer = (props: ExpandPanelProps) => {
  ...
  return (
      ...
      <Panel  collapsed={!expanded} onToggle={(e) => handleExpansion(childPath)}>
             <JsonFormsDispatch
                 schema={schema}
                 uischema={foundUISchema}
                 path={childPath}
...
): ComponentType<StatePropsOfExpandPanel> => ({
  ctx,
  props
}: JsonFormsStateContext & StatePropsOfExpandPanel) => {
...
export const withJsonFormsPanelProps = (
  Component: ComponentType<ExpandPanelProps>
...
export default withJsonFormsPanelProps(PanelRenderer);
