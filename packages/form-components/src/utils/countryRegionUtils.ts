...
export const getRegions = (countryShortCode: string): Region[] | undefined =>
    CountryRegionData.find((country) => country.countryShortCode === countryShortCode)?.regions;
...
export const filterCountries = (props: FilterCountriesProps) => {
    ...
    if (priorityCountries && priorityCountries.length > 0) {
        ...
        filteredCountries = filteredCountries.filter(
            (country) => priorityCountries.indexOf(country.countryShortCode) === -1
        );
    }
