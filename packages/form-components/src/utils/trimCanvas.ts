...
const getAlpha = (x: number, y: number, imgWidth: number, imgData: Uint8ClampedArray) => {
    return getRGBA(x, y, imgWidth, imgData).alpha
}

// finds the first y coord in imgData that is not white
const scanY = (fromTop: boolean, imgWidth: number, imgHeight: number, imgData: Uint8ClampedArray): number | undefined => {
...
const scanX = (fromLeft: boolean, imgWidth: number, imgHeight: number, imgData: Uint8ClampedArray): number | undefined => {
    const firstRow = fromLeft ? 0 : imgWidth - 1
    // loop through each column
    for (let x = firstRow; fromLeft ? (x < imgWidth) : (x > -1); x += fromLeft ? 1 : -1) {
        // loop through each row
