...
const OptionsLayout: FunctionComponent<ArrayControlProps> = ({ path, data }) => {
  ...
  const labelEditor = (props: RowProps) => (
    <InputText value={edits[props.rowData.id].label} onChange={(e) => editLabel(props.rowData.id, e.target.value)} />
  );

  const valueEditor = (props: RowProps) =>
    optionValueType === 'string' ? (
      <InputText value={edits[props.rowData.id].value} onChange={(e) => editValue(props.rowData.id, e.target.value)} />
    ) : (
      <InputNumber
