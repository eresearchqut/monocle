...
const sortLast = ['#/properties/inputs', '#/properties/options', , '#/properties/sections'];

const scopeOrder = (scope: string): number =>
    sortLast.indexOf(scope) >= 0 ? 10000 : sortFirst.indexOf(scope) >= 0 ? sortFirst.indexOf(scope) : 100;
...
const VerticalLayoutRenderer: FunctionComponent<LayoutProps> = ({
    ...
    return (
        <div className="p-grid p-dir-col p-fluid">{renderChildren(verticalLayout, schema, path, renderers, cells)}</div>
    );
};
